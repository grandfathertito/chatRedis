...
class ApiController extends Controller
{
     ...
     * @return \Response
     */
    public function respondForbidden($message = 'The Request Is Understood, But It Has Been Refused!')
    {
    ...
    /**
     * Returned in when a request cannot be served due to the application’s rate limit
    ...
    /**
     * The Application servers are up, but the request couldn’t be serviced due to some failure
     * within our stack. Try again later.
     *
     * @param string $message
     * @return \Response
     */
    ...
    function successResponseWeb($data = null, $msg = 'OK', $status_code = 200, $status = true)
    {
        $response = [
            'status' => $status,
            'code' => $status_code,
    ...
    function uploadImage($photo, $path)
    {
        ...
        $photo->storeAs('public/' . $path, $image_name);
        return $image_name;
    }

    function removeImage($image_name, $path)
    {
